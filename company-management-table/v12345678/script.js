...
// Partie 6 : trie les entreprises par ordre alphabétique sur le nom.
// Le tri est fait sur le tableau companies lui-même (tri en place).
function sortByName() {
  companies.sort((a, b) => a.company.localeCompare(b.company))
}

// Partie 6 : trie les entreprises par revenu croissant.
function sortByRevenueAsc() {
  companies.sort((a, b) => a.revenue - b.revenue)
}

// Partie 6 : trie les entreprises par revenu décroissant.
function sortByRevenueDesc() {
  companies.sort((a, b) => b.revenue - a.revenue)
}

// Partie 6 : trie les entreprises par date de création (ordre croissant).
// createdAt est au format ISO "YYYY-MM-DD", donc la comparaison de chaînes
// donne déjà l'ordre chronologique.
function sortByCreatedAt() {
  companies.sort((a, b) => a.createdAt.localeCompare(b.createdAt))
}

// Partie 8 : retourne la somme totale des revenus de toutes les entreprises.
function getTotalRevenue() {
  return companies.reduce((total, company) => total + company.revenue, 0)
}

// Partie 8 : retourne la moyenne des revenus.
// On renvoie 0 si le tableau est vide pour éviter une division par zéro.
...
// Partie 8 : retourne le nombre d'entreprises actives.
function countActiveCompanies() {
  return getActiveCompanies().length
}

// Partie 8 : retourne le nombre d'entreprises appartenant au pays donné.
...
// Partie 8 : met à jour l'affichage des statistiques globales.
// La moyenne est arrondie à 2 décimales pour rester lisible.
...
function renderCompanies(list) {
  ...
  // Partie 8 : les statistiques sont toujours calculées sur le tableau
  // complet (companies), indépendamment du filtre d'affichage appliqué.
  renderStats();
}
...
// Partie 6 : branchement des en-têtes de colonnes sur les fonctions de tri.
// - Clic sur "Created At" : sortByCreatedAt()
// - Clic sur "Company"    : sortByName()
// - Clic sur "Revenue"    : sortByRevenueAsc() / sortByRevenueDesc() en bascule
// Seul le revenu alterne croissant/décroissant ; les autres restent croissants.
